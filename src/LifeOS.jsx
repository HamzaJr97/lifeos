...
export default function LifeOS() {
  ...
  const [settings, setSettings] = useLocalStorage('los_settings',{
    name:'Hero', currency:'€', language:'en', incomeTarget:3000, savingsTarget:20, ollamaUrl:'http://localhost:11434', aiProvider:'ollama', pin:'', showPin:false
  });
...
/* iOS safe-area: fill ALL four edges with app background — kills white bars */
html {
  background: ${T.bg} !important;
}
body {
  background: ${T.bg} !important;
  /* Pad body INTO the safe areas so background shows, not white */
...
/* Cover all 4 edges with pseudo-elements as a safety net */
...
Max 280 words.`
    ...
    if (!apiKey) {
      setAiCoachResult('⚠️ No API key configured. Go to ⚙️ Settings → Anthropic API Key and paste your sk-ant-... key to unlock the AI coach.');
    ...
    try {
      ...
      const data = await res.json();
      if (data.error) setAiCoachResult('⚠️ API Error: ' + data.error.message);
      else setAiCoachResult(data.content?.[0]?.text || 'No response from Claude.');
    } catch(err) {
      setAiCoachResult('⚠️ Could not reach Claude API: ' + err.message);
    }
    ...
    <div style={{display:'flex',flexDirection:'column',gap:'20px'}}>
        ...
        <div style={{position:'fixed',inset:0,background:'#00000088',display:'flex',alignItems:'center',justifyContent:'center',zIndex:600}} onClick={()=>setShowAiCoach(false)}>
          <div style={{...s.card,width:'620px',maxWidth:'96vw',maxHeight:'90vh',overflowY:'auto',border:`1px solid ${T.accent}55`}} onClick={e=>e.stopPropagation()}>
            ...
            <button style={{...s.btn(T.accent),width:'100%',padding:'13px',fontSize:'14px',fontWeight:'700',marginBottom:'14px',letterSpacing:'0.3px'}}
              ...
              {aiCoachLoading ? '🤖 Claude is analyzing your finances...' : `🚀 Analyze My ${aiCoachTab==='behavior'?'Spending Behavior':aiCoachTab==='savings'?'Savings Potential':'Debt Strategy'}`}
            </button>
            ...
            {!settings.anthropicKey && (
              <div style={{background:T.warning+'18',border:`1px solid ${T.warning}44`,borderRadius:'8px',padding:'10px 12px',marginBottom:'12px',fontSize:'12px',color:T.warning}}>
                ⚙️ Add your Anthropic API key in <strong>Settings</strong> to enable this coach. It's free to try — get one at console.anthropic.com
              </div>
            )}
      ...
      <div style={{background:`linear-gradient(135deg,${T.accent}18 0%,${T.surface} 100%)`,border:`1px solid ${T.accent}44`,borderRadius:'14px',padding:'16px 20px',display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:'12px'}}>
        <div>
          ...
          {!settings.anthropicKey && <div style={{fontSize:'10px',color:T.warning,marginTop:'4px'}}>⚙️ Add API key in Settings to unlock</div>}
        </div>
        ...
        <div style={s.card}>
          ...
          <div style={{display:'flex',gap:'8px',marginBottom:'16px'}}>
            {['ollama','anthropic'].map(p=>(
              <button key={p} style={{...s.btnGhost,background:settings.aiProvider===p?T.accentSoft:'transparent',color:settings.aiProvider===p?T.accent:T.textMuted,textTransform:'capitalize'}} onClick={()=>setSettings(ss=>({...ss,aiProvider:p}))}>{p==='ollama'?'🦙 Ollama (Free)':'🤖 Anthropic API'}</button>
            ))}
          </div>
